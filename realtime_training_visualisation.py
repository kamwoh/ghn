...
def get_layers_and_weights(model):
    ...
    weights = [weight.name for weight in model.trainable_weights]
    layers_dict = to_dict(layers)
    weights_dict = filter_conv(to_dict(weights), model)
    weights = map(lambda weight: weight.name,
                  filter(lambda weight: len(weight.shape) == 4, model.trainable_weights))
    return layers, weights, layers_dict, weights_dict
...
def main():
    ...
    with graph.as_default():
        ...
        with sess.as_default():
            ...
            # print(ghd_model.model.loss_functions)
            # if os.path.exists('./keras_mnist_ghd.h5'):
            #     ghd_model.load_weights('./keras_mnist_ghd.h5')
            ghd_relu_model = ghd_mnist_model(learning_rate, double_threshold, per_pixel, alpha, relu=True)
            ...
            ghd_layers, ghd_weights, ghd_layers_dict, ghd_weights_dict = get_layers_and_weights(ghd_model)
            ghd_relu_layers, ghd_relu_weights, ghd_relu_layers_dict, ghd_relu_weights_dict = get_layers_and_weights(
                ghd_relu_model)
            # bn_layers, bn_weights, bn_layers_dict, bn_weights_dict = get_layers_and_weights(bn_model)
            # naive_layers, naive_weights, naive_layers_dict, naive_weights_dict = get_layers_and_weights(naive_model)

            layer_choices = LayerChoices()
            layer_choices.add_choices('ghd', ghd_layers)
            layer_choices.add_choices('ghd_weights', ghd_weights)
            layer_choices.add_choices('ghd_relu', ghd_relu_layers)
            layer_choices.add_choices('ghd_relu_weights', ghd_relu_weights)
            ...
            realtime_model.add_model('ghd_relu', ghd_relu_model, ghd_relu_layers_dict, ghd_relu_weights_dict)
